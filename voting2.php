...
body
{
  background-image: url('pic/c11.png');
...
<body>
	...
	<a class="brand">
	 <h2>Vits College Elections</h2>
	 <div class="chmsc_nav"><font size="4" color="white">Vignan Institute Of Technology And Science</font></div>
 	</a>
...
<img class="gov" src="<?php echo $row['Photo'];?>" width="150" height="150" border="0" onmouseover="showtrail('<?php echo $row['Photo'];?>','<?php echo $row['FullName'];?> ',200,5)" onmouseout="hidetrail()">
&nbsp;&nbsp;&nbsp;&nbsp;
...
$governor=mysqli_query($conn,"select * from candidate where Position='Governor'")or die(mysqli_error());
while($row=mysqli_fetch_array($governor)){ $governor_id=$row['CandidateID']; ?>
<option value="<?php echo $governor_id; ?>" class="option"><?php  echo $row['FullName']; ?></option>
<?php } ?>
</select>
</div>
</div>
...
$vice=mysqli_query($conn,"select * from candidate where Position='Vice-Governor'")or die(mysqli_error());
while($row=mysqli_fetch_array($vice)){ $vice_id=$row['CandidateID']; ?>
<option value="<?php echo $vice_id; ?>" class="option"><?php  echo $row['FullName']; ?></option>
<?php } ?>
</select>
</div>
</div>
...
<?php 
$representative=mysqli_query($conn,"select * from candidate where Position='2nd Year Representative' order by FullName Asc")or die(mysqli_error());
while($row=mysqli_fetch_array($representative)){ $representative_id=$row['CandidateID']; ?>

<img class="gov" src="<?php echo $row['Photo'];?>" width="70" height="70" border="0" onmouseover="showtrail('<?php echo $row['Photo'];?>','<?php echo $row['FullName'];?> ',200,5)" onmouseout="hidetrail()">
&nbsp;&nbsp;&nbsp;&nbsp;
...
$representative=mysqli_query($conn,"select * from candidate where Position='2nd Year Representative' order by FullName Asc")or die(mysqli_error());
while($row=mysqli_fetch_array($representative)){ $representative_id=$row['CandidateID']; ?>
<option value="<?php echo $representative_id; ?>" class="option1"><?php  echo $row['FullName']; ?></option>
<?php } ?>
</select>
</div>
